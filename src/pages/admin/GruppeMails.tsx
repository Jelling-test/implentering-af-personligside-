...
import { Mail, Search, Trash2, Send, Users, Plus, UserPlus, CalendarDays } from "lucide-react";
import {
  Dialog,
  DialogContent,
  DialogDescription,
...
const AdminGruppeMails = () => {
  ...
  // Tilføj mail dialog states
  const [showAddDialog, setShowAddDialog] = useState(false);
  const [newEmail, setNewEmail] = useState("");
  const [newName, setNewName] = useState("");
  const [addLoading, setAddLoading] = useState(false);

  // Periode-baseret mail states
  const [periodStart, setPeriodStart] = useState("");
  const [periodEnd, setPeriodEnd] = useState("");
  const [periodRecipientCount, setPeriodRecipientCount] = useState<number | null>(null);
  ...
  const addSubscriber = async () => {
    ...
    try {
      ...
      const { error } = await (supabase as any)
        .from("email_subscribers")
        .insert({
          customer_name: newName.trim() || "Nyhedsbrev",
          email: newEmail.trim().toLowerCase(),
          customer_type: "nyhedsbrev",
          year: currentYear,
          checked_in: false,
        });
      ...
      if (error) {
        if (error.code === "23505") {
          toast.error("Email findes allerede i databasen");
        } else {
          throw error;
        }
      } else {
        toast.success("Email tilføjet til nyhedsbrev");
        setNewEmail("");
        setNewName("");
      ...
      toast.error("Fejl ved tilføjelse af email");
    } finally {
      setAddLoading(false);
    }
  };
  ...
  const getRecipients = async (type: string, year?: string) => {
    ...
    switch (type) {
      ...
      case "upcoming":
        // Kommende kunder - ankomst efter i dag
        const today = new Date().toISOString().split('T')[0];
        const { data: upcomingRegular } = await (supabase as any)
          .from("regular_customers")
          .select("email")
          .gt("arrival_date", today)
          .not("email", "is", null);
        
        const { data: upcomingSeasonal } = await (supabase as any)
          .from("seasonal_customers")
          .select("email")
          .gt("arrival_date", today)
          .not("email", "is", null);
        ...
        const upcomingEmails = [
          ...(upcomingRegular?.map((c: any) => c.email) || []),
          ...(upcomingSeasonal?.map((c: any) => c.email) || [])
        ];
        return [...new Set(upcomingEmails)];
  ...
  // Hent kunder med ankomst i en periode
  const getRecipientsInPeriod = async () => {
    if (!periodStart || !periodEnd) return [];

    // Hent fra regular_customers
    const { data: regularCustomers } = await (supabase as any)
      .from("regular_customers")
      .select("email")
      .gte("arrival_date", periodStart)
      .lte("arrival_date", periodEnd)
      .not("email", "is", null);
    
    // Hent fra seasonal_customers
    const { data: seasonalCustomers } = await (supabase as any)
      .from("seasonal_customers")
      .select("email")
      .gte("arrival_date", periodStart)
  ...
  // Opdater antal modtagere i periode
  const updatePeriodRecipientCount = async () => {
    if (!periodStart || !periodEnd) {
      setPeriodRecipientCount(null);
      return;
    }
    const recipients = await getRecipientsInPeriod();
    setPeriodRecipientCount(recipients.length);
  };

  // Send til kunder i periode
  const sendToPeriod = async () => {
    if (!emailSubject.trim() || !emailBody.trim()) {
      toast.error("Udfyld venligst emne og besked");
      return;
    }
    ...
    try {
      ...
      if (recipients.length === 0) {
        toast.error("Ingen kunder med ankomst i denne periode");
        setLoading(false);
        return;
      }

      if (!confirm(`Er du sikker på at du vil sende email til ${recipients.length} kunder med ankomst ${periodStart} - ${periodEnd}?`)) {
        setLoading(false);
        return;
      }
  ...
  return (
    <SidebarProvider>
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                  ...
                  <Card>
                    ...
                    <CardContent className="space-y-2">
                      ...
                      <Button
                        onClick={() => sendGroupEmail("upcoming")}
                        disabled={loading}
                        className="w-full"
                        variant="outline"
                      >
                  ...
                  {/* Periode-baseret mail */}
                  <Card>
                    <CardHeader className="pb-3">
                      <CardTitle className="text-sm">Ankomst i Periode</CardTitle>
                    </CardHeader>
                    <CardContent className="space-y-2">
                      <div className="space-y-2">
                      ...
                      <Button
                        onClick={updatePeriodRecipientCount}
                      ...
                      {periodRecipientCount !== null && (
                        <p className="text-xs text-center text-muted-foreground">
                          {periodRecipientCount} kunder fundet
                        </p>
                      )}
                      <Button
                        onClick={sendToPeriod}
      ...
      <Dialog open={showAddDialog} onOpenChange={setShowAddDialog}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>Tilføj til nyhedsbrev</DialogTitle>
            <DialogDescription>
              Tilføj en ny email adresse til nyhedsbrevet
            </DialogDescription>
          </DialogHeader>
          <div className="space-y-4 py-4">
            <div className="space-y-2">
              <Label htmlFor="add-name">Navn (valgfrit)</Label>
              <Input
                id="add-name"
            ...
            <div className="space-y-2">
              <Label htmlFor="add-email">Email adresse *</Label>
              <Input
                id="add-email"
                type="email"
          ...
          <DialogFooter>
            ...
            <Button onClick={addSubscriber} disabled={addLoading}>
              {addLoading ? "Tilføjer..." : "Tilføj email"}
            </Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
