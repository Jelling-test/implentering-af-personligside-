...
const AdminRapporter = () => {
  ...
  // Meter history states
  const [meterSearchQuery, setMeterSearchQuery] = useState("");
  const [meterHistoryStartDate, setMeterHistoryStartDate] = useState<Date>(new Date(Date.now() - 30 * 24 * 60 * 60 * 1000));
  const [meterHistoryEndDate, setMeterHistoryEndDate] = useState<Date>(new Date());
  const [meterHistoryData, setMeterHistoryData] = useState<any[]>([]);
  const [meterHistoryInfo, setMeterHistoryInfo] = useState<{meterNumber: string; ieeeAddress: string | null} | null>(null);
  ...
  const searchMeterHistory = async () => {
    if (!meterSearchQuery.trim()) {
      toast.error("Indtast et måler-nummer eller IEEE adresse");
      return;
    }

    setLoading(true);
    try {
      const startStr = format(meterHistoryStartDate, "yyyy-MM-dd");
      const endStr = format(meterHistoryEndDate, "yyyy-MM-dd");
      const query = meterSearchQuery.trim();

      // Først: Find måler info via meter_identity (søg på både meter_number og ieee_address)
      const { data: identityData } = await (supabase as any)
        .from("meter_identity")
        .select("meter_number, ieee_address")
        .or(`meter_number.ilike.%${query}%,ieee_address.ilike.%${query}%`)
        .limit(1)
        .single();

      let meterNumber = query;
      let ieeeAddress: string | null = null;

      if (identityData) {
        meterNumber = identityData.meter_number;
        ieeeAddress = identityData.ieee_address;
      }

      setMeterHistoryInfo({ meterNumber, ieeeAddress });

      // Hent historik data fra meter_readings_history (daglige snapshots kl 23:59)
      const { data: historyData, error } = await (supabase as any)
        .from("meter_readings_history")
        .select("meter_id, time, energy, power, voltage, current, state")
        .eq("meter_id", meterNumber)
        .eq("snapshot_time", "23:59")
        .gte("time", startStr + "T00:00:00")
        .lte("time", endStr + "T23:59:59")
        .order("time", { ascending: true });

      if (error) throw error;

      // Beregn dagligt forbrug (difference i energy)
      const dailyData = [];
      for (let i = 1; i < (historyData || []).length; i++) {
        const prev = historyData[i - 1];
        const curr = historyData[i];
        const consumption = Math.max(0, (curr.energy || 0) - (prev.energy || 0));
        dailyData.push({
          date: curr.time,
          energy: curr.energy,
      ...
      setMeterHistoryData(dailyData);
      
      if (dailyData.length === 0) {
        toast.info(`Ingen historik fundet for "${meterNumber}" i den valgte periode`);
      } else {
        toast.success(`Fundet ${dailyData.length} dages historik for "${meterNumber}"`);
      }
    } catch (error) {
      console.error("Error fetching meter history:", error);
      toast.error("Fejl ved søgning i måler historik");
    } finally {
      setLoading(false);
    }
  };

  const getMeterHistoryChartData = () => {
    if (!meterHistoryData.length) return null;

    const labels = meterHistoryData.map((d) =>
      format(new Date(d.date), "dd/MM", { locale: da })
    );

    // Beregn kumulativt forbrug (akkumuleret over tid)
    let cumulative = 0;
    const cumulativeData = meterHistoryData.map((d) => {
      cumulative += d.consumption;
  ...
  const getMeterHistoryChartOptions = () => ({
    responsive: true,
    plugins: {
      legend: {
      ...
      tooltip: {
        callbacks: {
          title: (context: any) => {
            ...
            if (idx !== undefined && meterHistoryData[idx]) {
              return format(new Date(meterHistoryData[idx].date), "dd. MMMM yyyy", { locale: da });
            }
            return "";
          },
          label: (context: any) => {
            const idx = context.dataIndex;
            if (meterHistoryData[idx]) {
              // Beregn kumulativt op til denne dag
              let cumulative = 0;
              for (let i = 0; i <= idx; i++) {
                cumulative += meterHistoryData[i].consumption;
              }
              return [
                `Akkumuleret: ${cumulative.toFixed(2)} kWh`,
                `Dagens forbrug: ${meterHistoryData[idx].consumption} kWh`,
              ];
            }
            return "";
          },
      ...
      <div className="min-h-screen flex w-full animate-fade-in">
        ...
        <div className="flex-1 flex flex-col">
          ...
          <main className="flex-1 p-4 sm:p-6 bg-muted/20 space-y-4 sm:space-y-6 overflow-auto">
            ...
            {/* Meter History Search */}
            <Card>
              <CardHeader>
                <CardTitle className="flex items-center gap-2">
                  <Zap className="h-5 w-5" />
                  Måler Historik Søgning
                </CardTitle>
                <CardDescription>Søg på måler-nummer eller IEEE adresse for at se forbrugshistorik</CardDescription>
              </CardHeader>
              <CardContent className="space-y-4">
                <div className="flex flex-col sm:flex-row items-start sm:items-center gap-4">
                  <div className="relative flex-1 w-full sm:max-w-xs">
                    ...
                    <Input
                      placeholder="Søg måler (fx 'F43' eller '0x781c...')"
                      value={meterSearchQuery}
                      onChange={(e) => setMeterSearchQuery(e.target.value)}
                      onKeyDown={(e) => e.key === "Enter" && searchMeterHistory()}
                      className="pl-9"
                    />
                  </div>
                  <Popover>
                    <PopoverTrigger asChild>
                      <Button variant="outline" className="w-[180px] justify-start text-left">
                        <CalendarIcon className="mr-2 h-4 w-4" />
                        {format(meterHistoryStartDate, "dd/MM/yyyy")}
                      </Button>
                    </PopoverTrigger>
                    <PopoverContent className="w-auto p-0 bg-background z-50" align="start">
                      <Calendar
                        mode="single"
                        selected={meterHistoryStartDate}
                        onSelect={(date) => date && setMeterHistoryStartDate(date)}
                  ...
                  <span className="text-muted-foreground hidden sm:inline">til</span>
                  <Popover>
                    <PopoverTrigger asChild>
                      <Button variant="outline" className="w-[180px] justify-start text-left">
                        <CalendarIcon className="mr-2 h-4 w-4" />
                        {format(meterHistoryEndDate, "dd/MM/yyyy")}
                      </Button>
                    </PopoverTrigger>
                    <PopoverContent className="w-auto p-0 bg-background z-50" align="start">
                      <Calendar
                        mode="single"
                        selected={meterHistoryEndDate}
                        onSelect={(date) => date && setMeterHistoryEndDate(date)}
                        disabled={(date) => date > new Date()}
                        initialFocus
                        className={cn("p-3 pointer-events-auto")}
                      />
                  ...
                  <Button onClick={searchMeterHistory} disabled={loading}>
                ...
                {meterHistoryInfo && (
                  <div className="flex items-center gap-4 p-3 bg-muted/50 rounded-lg">
                    <div>
                      ...
                      <span className="ml-2 font-medium">{meterHistoryInfo.meterNumber}</span>
                    </div>
                    {meterHistoryInfo.ieeeAddress && (
                      <div>
                        <span className="text-sm text-muted-foreground">IEEE:</span>
                        <span className="ml-2 font-mono text-sm">{meterHistoryInfo.ieeeAddress}</span>
                      </div>
                    )}
                  </div>
                )}
                ...
                {/* Chart */}
                {meterHistoryData.length > 0 && getMeterHistoryChartData() && (
                  <div className="space-y-4">
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                      <Card>
                        <CardHeader className="pb-2">
                          <CardTitle className="text-sm">Total forbrug i perioden</CardTitle>
                        </CardHeader>
                        <CardContent>
                          <p className="text-2xl font-bold">
                            {meterHistoryData.reduce((sum, d) => sum + d.consumption, 0).toFixed(1)} kWh
                          </p>
                        </CardContent>
                      </Card>
                      <Card>
                        ...
                        <CardContent>
                          <p className="text-2xl font-bold">
                            {(meterHistoryData.reduce((sum, d) => sum + d.consumption, 0) / meterHistoryData.length).toFixed(2)} kWh
                          </p>
                        </CardContent>
                      </Card>
                      <Card>
                        <CardHeader className="pb-2">
                          <CardTitle className="text-sm">Dage med data</CardTitle>
                        </CardHeader>
                        <CardContent>
                          <p className="text-2xl font-bold">{meterHistoryData.length}</p>
                        </CardContent>
                      </Card>
                      <Card>
                        <CardHeader className="pb-2">
                          <CardTitle className="text-sm">Målerens totale forbrug</CardTitle>
                        </CardHeader>
                        <CardContent>
                          <p className="text-2xl font-bold">
                            {meterHistoryData[meterHistoryData.length - 1]?.energy?.toFixed(1) || 0} kWh
                          </p>
                        </CardContent>
                      </Card>
                    </div>
                    ...
                    <Card>
                      <CardHeader>
                        <CardTitle className="text-sm">Dagligt forbrug over tid</CardTitle>
                      </CardHeader>
                      <CardContent>
                        <Line options={getMeterHistoryChartOptions()} data={getMeterHistoryChartData()!} />
                      </CardContent>
                    </Card>
                  </div>
                )}

                {meterHistoryData.length === 0 && meterHistoryInfo && (
                  <div className="text-center py-8 text-muted-foreground">
                    Ingen historik fundet for den valgte periode
                  </div>
                )}

                {!meterHistoryInfo && (
                  <div className="text-center py-8 text-muted-foreground">
                    Indtast et måler-nummer eller IEEE adresse og klik "Søg" for at se forbrugshistorik
                  </div>
                )}
              </CardContent>
            </Card>
